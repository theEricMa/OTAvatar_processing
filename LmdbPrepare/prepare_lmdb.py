...
def get_inv(file_path):
    ...
    angles *= -1
    ids = file_mat['id']
    condtion = np.hstack([angles, translation, focal.repeat(angles.shape[0])[..., None]], )
    return condtion.tobytes(), ids.tobytes()
    # Rs = compute_rotation(torch.from_numpy(angles)).numpy()
    # c_list = process_camera(translation, Rs, focal)
    # return np.vstack(c_list).tobytes()
...
def compute_rotation(euler_angle):
...
class Resizer:
    def __init__(self, size, kp_root, coeff_3dmm_root, mat_root, inv_3dmm_root, img_format):
        self.size = size
        self.kp_root = kp_root
        self.coeff_3dmm_root = coeff_3dmm_root
        self.mat_root = mat_root
        self.img_format = img_format
        self.inv_3dmm_root = inv_3dmm_root
    ...
    def prepare(self, filename):
        frames = {'img':[], 'img_mat':[],'kp':None, 'coeff_3dmm':None, 'inv_3dmm':None, 'id':None}
        ...
        condition_byte, id_byte = get_others(self.inv_3dmm_root, video_name, 'inv_3dmm')
        frames['kp'] = keypoint_byte
        frames['coeff_3dmm'] = coeff_3dmm_byte
        frames['condition'] = condition_byte
        frames['id'] = id_byte
        return frames
...
def get_others(root, video_name, data_type):
    if root is None:
        ...
        assert data_type in ('keypoint', 'coeff_3dmm', 'inv_3dmm')

    if data_type in ['coeff_3dmm','inv_3dmm']:
        ext = '.mat'
    ...
    if os.path.exists(os.path.join(root, 'train', video_name+ext)):
        file_path = os.path.join(root, 'train', video_name+ext)
    elif os.path.exists(os.path.join(root, 'test', video_name+ext)):
        file_path = os.path.join(root, 'test', video_name+ext)
    else:
        print(os.path.join(root, 'train', video_name+ext), os.path.join(root, 'test', video_name+ext), )
        raise ValueError
    
    if data_type == 'keypoint':
        return_byte = convert_kp(file_path)
    elif data_type == 'coeff_3dmm':
        return_byte = convert_3dmm(file_path)
    elif data_type == 'inv_3dmm':
        return_byte = get_inv(file_path)
    else:
        raise ValueError
...
def convert_kp(file_path):
    kp_byte = np.loadtxt(file_path).astype(np.float32).tobytes()
    return kp_byte
...
def prepare_data(path, keypoint_path, coeff_3dmm_path, matting_path, inv_3dmm_path, out, n_worker, sizes, chunksize, img_format):
    filenames = list()
    ...
    # filter files
    filenames = [f for f in filenames if os.path.exists(f.replace('matting', 'kp').replace('mp4', 'txt'))]
    print(len(filenames))
    filenames = [f for f in filenames if os.path.exists(f.replace('matting', '3dmm').replace('mp4', 'mat'))]
    print(len(filenames))
    filenames = [f for f in filenames if os.path.exists(f.replace('matting', 'inv_render').replace('mp4', 'mat'))]
    print(len(filenames))
    ...
    for size in sizes:
        ...
        with lmdb.open(lmdb_path, map_size=1024 ** 4, readahead=False) as env:
            with env.begin(write=True) as txn:
                ...
                resizer = Resizer(size, keypoint_path, coeff_3dmm_path, matting_path, inv_3dmm_path, img_format)
                # tmp = resizer((0, filenames[0]))
                ...
                with multiprocessing.Pool(n_worker) as pool:
                    for idx, result, filename in tqdm(
                        ...
                        if result['condition']:
                            txn.put(format_for_lmdb(video_name, 'condition'), result['condition'] )
                        if result['id']:
                            txn.put(format_for_lmdb(video_name, 'id'), result['id'] )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--matting_path', type=str, help='a path to output directory', default=None)
    parser.add_argument('--coeff_3dmm_path', type=str, help='a path to output directory', default=None)
    parser.add_argument('--inv_3dmm_path', type=str, help='a path to output directory', default=None)
