...
def get_data_path(root, keypoint_root):
    ...
    # # found matches 
    # filenames = [f for f in filenames if f.replace('hdtf', 'hdtf_kp').replace('mp4', 'txt') in keypoint_filenames]
...
class VideoPathDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        out_images, out_trans_params, out_warpings = list(), list(), list()
...
def main(opt, model):
    ...
    for data in tqdm(dataloader):
        ...
        num_batch = math.ceil(data['imgs'][0].shape[0] / batch_size)
        pred_coeffs = list()
        warping_params = list()
