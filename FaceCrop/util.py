...
def crop_bbox_from_frames_with_fix(frame_list, tube_bbox, min_frames=16, image_shape=(256, 256), min_size=200,
                          increase_area=0.1, aspect_preserving=True):
    frame_shape = frame_list[0].shape
    # Filter short sequences
    if len(frame_list) < min_frames:
    ...
    if max(left_oob / float(width), right_oob / float(width), top_oob  / float(height), bot_oob / float(height)) > 0:
        ...
        for frame in frame_list:
            out = np.zeros([bot - top, right - left, 3]).astype(np.uint8)
            out[top_oob: bot - top - bot_oob, left_oob: right - left - right_oob] =  frame[max(0, top): min(bot, frame_shape[0]), max(left, 0):min(right,frame_shape[1])]
            selected.append(out)
    else:
        selected = [frame[top:bot, left:right] for frame in frame_list]
...
def scheduler(data_list, fn, args):
    ...
    fn((data_list[0], device_ids[0], args))
    exit(0)
...
def save(path, frames, format, audio = None):
    ...
    if format == '.mp4':
        ...
        tmp_path = os.path.join(dir, 'tmp_'+name)
        # save the output
        if audio is None:
            ffmpeg.filter(video, 'fps', fps=30, round='up').output(tmp_path).run(overwrite_output=True)
        else:
            ffmpeg.concat(video, audio, v=1, a=1).filter('fps', fps=30, round='up').output(tmp_path).run(overwrite_output=True)
